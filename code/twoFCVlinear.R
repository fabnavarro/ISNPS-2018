twoFCVlinear <- function(n, Y, j0, qmf, D, wc, sigma_v) {
  n1 <- n/2
  lD <- length(D)
  wc_odd_lin <- rep(0, n1)
  ...
  for (i in 1:length(D)) {
    wc_hat_s_m[1:(2^i)] <- wc[1:(2^i)] - sigma_v^2 * 2^(-i/2)
  ...
  tmp <- (mat_Y_odd - bar_s_even)^2 + (mat_Y_even - bar_s_odd)^2
  Crit_CV <- rowSums(tmp)
  return(list(CritCV = Crit_CV, hat_s_m = hat_s_m))
}
